...
- `src/`
  ...
  - `image_auto.py`: SAM2 자동 마스크 생성기(옵션: depth 기반 추가 필터링)
...
- 메시 출력(`glb`/`obj`/`ply`)은 모두 동일하게 x축 기준 반시계 +90° 회전을 적용해 저장합니다.
...
./run_full_pipeline.sh \
  ...
  --output-base outputs/demo
```
- `--sam2d-method sam2d_auto`에서 `--depth-image`가 주어지면, `image_auto.py`에 depth가 전달되어 중복/경계/nested-depth 필터가 함께 적용됩니다.
- `--depth-image`가 없으면 depth 의존 필터는 건너뛰고 자동 마스크 기본 필터만 적용됩니다.
...
자주 쓰는 옵션 요약:
- `--image`: 입력 RGB 이미지
- `--sam2d-method`: SAM2 마스크 방식 선택 (`sam2d_point` | `sam2d_auto`, 기본: `sam2d_point`)
- `--depth-image`: real depth 이미지(있으면 real_scale 생성)
- `--cam-k`: 3x3 intrinsics 텍스트 파일
- `--depth-scale`: depth 스케일 (`0.001` 기본, 필요 시 `auto` 또는 다른 numeric 값 지정)
...
`image_auto.py` 직접 실행 시 자주 쓰는 추가 옵션:
- `--depth-image`: depth 기반 필터 활성화
- `--dedupe-iou-thresh`: 중복 마스크 제거 IoU 임계값 (기본: 0.85)
- `--depth-same-surface-thresh`: nested depth 연결 임계값(미지정 시 자동 추정)
- `--keep-largest-component` / `--no-keep-largest-component`
- `--filter-border` / `--no-filter-border`
- `--apply-extra-filtering` / `--no-apply-extra-filtering`
...
- `ImportError/ModuleNotFoundError: sam2._C` 또는 `Skipping the post-processing step...`
  - `sam2._C` CUDA 확장 빌드 실패 상태입니다.
  - PyTorch CUDA 버전(`torch.version.cuda`)과 `nvcc` 버전을 맞추세요(예: 둘 다 12.1).
  - 빌드시 conda GCC(14) 충돌이 나면 `CC/CXX/CUDAHOSTCXX`를 시스템 `gcc-12/g++-12`로 지정해 `python setup.py build_ext --inplace`를 실행하세요.
